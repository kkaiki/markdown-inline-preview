...
## 1. 実 VS Code 拡張ホスト（`@vscode/test-electron`） — 84 件
...
## 2. 実 Chromium ブラウザ（Playwright + 実 webview バンドル）— すべて Live — 141 件
...
## 3. ユニット・純関数（jsdom）— live/ raw/ shared/ に分類 — 733 件
...
### `test/suite/live/shortcuts/selectAllScope.test.ts`（9 件）
