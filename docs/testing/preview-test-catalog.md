...
## 2. 実 Chromium ブラウザ（Playwright + 実 webview バンドル）— すべて Preview — 457 件
...
### `test/browser/live/rendering/phase5Rendering.test.ts`（13 件）
...
- **Live モード: Phase 5 の描画（実ブラウザ）**
  ...
  - **数式ブロック（ブロックスコープ）**
    ...
    - ブロックの中にカーソルを置くと生テキストに戻り、描画結果も併記される
    - 壊れた数式でもエディタが落ちない
  - **インライン数式（トークンスコープ）**
    - カーソルが外にあるとき KaTeX で描画される
