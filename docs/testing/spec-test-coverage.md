...
| `default-editor-association-sync.md` | `test/suite/preview/tabs-editors/defaultEditorAssociation.test.ts`（純関数15件）、`test/extension/preview/settings.test.ts` 16.1〜16.4（実 VS Code の設定追従）、`test/extension/preview/tabs-editors.test.ts` 17.1（Raw モードで Preview の Custom Editor が一度も生成されないことをタブ生成イベントで検証） | 実装済み（2026-07-26）。`workbench.editorAssociations` をモードへ同期させ、`.md` を開く経路での解決先を1つに確定させた。`preview-default-editor-fix.md` の「他拡張との priority 競合」「Raw ユーザーの逆方向ちらつき」の恒久対処。**適用範囲の限界**: `togglePreview` コマンド自体と `untitled:` は引き続き「開いてから閉じる」構造が残る（仕様書の該当節に明記） |
