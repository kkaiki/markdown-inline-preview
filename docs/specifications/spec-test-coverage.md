...
| `blank-line-preservation.md` | `test/webview/rendering/blankLineRoundtrip.test.ts`（連続空行のround-trip）、`test/browser/rendering/lineNumberGutter.test.ts`（実ソース行番号化・表/コードブロックの行内番号・空行スペーサーの行番号と入力/Backspace） | 新規実装・テスト済み（2026-07-08、行番号を2026-07-09に実ソース行番号版へ改訂）。トップレベルの空行を空 paragraph として復元し、ガター番号は Raw と一致する実ソース行番号（表・コードブロックは行ごと）を表示 |
