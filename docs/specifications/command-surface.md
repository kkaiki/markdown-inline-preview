...
| `/h1` `/h2` `/h3` `/h4` `/h5` `/h6` | 見出しを素早く挿入（Notion スタイル） | 提案 |
| `/table` | テーブルを作成または整形 | 実装済み |
| `/table normalize on\|off` | テーブル自動整形の切り替え | 実装済み |
| `/toc` | 目次を生成・更新 | 実装済み |
| `/目次` | `/toc` の日本語エイリアス | 実装済み |
| `/code [language]` | コードブロックを挿入 | 提案 |
| `/quote` | 引用ブロックを挿入 | 提案 |
| `/divider` | 水平線を挿入 | 提案 |
| `/callout` | コールアウト（注意書き）ブロックを挿入 | 提案 |
| `/bullet` | 箇条書きリストを開始 | 提案 |
| `/numbered` | 番号付きリストを開始 | 提案 |
| `/todo` | チェックボックスリストを開始 | 提案 |
