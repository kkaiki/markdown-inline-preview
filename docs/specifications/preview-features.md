...
| Git 差分ガター | Git HEAD（コミット済み）と比較し、ブロック左に 追加=緑 / 変更=青 のバー、削除位置に赤い線を表示（ブロック単位）。基準は開いた時点の HEAD。git 管理外/新規ファイルは非表示。**基準（HEAD 本文）も本文ドキュメントと同じ正規形（`normalizePreviewMarkdown`）に揃えてから比較する**ため、表セルの `<br>`（→ `&#10;`）等で Raw は無変更なのに Preview だけ青く見える誤検出は起きない。**フォーカスで記法展開（Typora 風）中のブロックは、展開中に実テキストとして挿入されている記法文字をシグネチャ比較から除外する**（`blockSignatures(doc, expandedRanges)`）。対象は 3 系統: 行頭プレフィックス（`## `/`- `/`> ` 等、`blockPrefixEditPlugin`）、インライン記法マーカー（`` ` ``/`**`/`*`/`~~`/`[`/`](url)`、`inlineMarkEditPlugin` の `getExpandedInlineMarkRanges()`）、コードフェンス行（`` ```lang ``/`` ``` ``、`codeFenceEditPlugin` の `getExpandedCodeFenceRanges()`）＝**フォーカスで実テキストを挿入するプラグインの全数**。除外しないと、見出し/箇条書き/blockquote やインラインコードを含む行にカーソルを合わせただけで（未編集でも）挿入された実テキスト分だけシグネチャが変わり、無変更のブロックがフォーカスした瞬間だけ「変更（青）」に見えてしまう（テーブルセル内のインラインコードでは、差分単位がトップレベルノードのためテーブル全体が青くなる）。展開プラグイン側は展開状態を `view.dispatch()` の**前**に確定させる（後にすると、その transaction を処理中の `decorations()` が空の展開状態を見て青バーが残る）。詳細: [inline-mark-focus-edit-fix.md](./fixes/inline-mark-focus-edit-fix.md) §3.2 |
