...
| チェックボックス | **クリックで常にトグル** → `- [x]` として保存。視覚的なチェックボックス（`label-wrapper`）は常に表示され、クリックで即座にトグルできる。`Cmd+Enter` でもトグル可。枠線は本文色から作るため（`--preview-checkbox-border`）テーマが薄くても見やすい。チェック済みは枠線スタイル（塗りつぶさず）＋本文色のチェックマーク。チェック済みの本文は少し暗く（`--preview-done-fg`）＋取り消し線（フォーカス中も維持） |
...
| Git 差分ガター | Git HEAD（コミット済み）と比較し、ブロック左に 追加=緑 / 変更=青 のバー、削除位置に赤い線を表示（ブロック単位）。基準は開いた時点の HEAD。git 管理外/新規ファイルは非表示。**基準（HEAD 本文）も本文ドキュメントと同じ正規形（`normalizePreviewMarkdown`）に揃えてから比較する**ため、表セルの `<br>`（→ `&#10;`）等で Raw は無変更なのに Preview だけ青く見える誤検出は起きない。Preview はフォーカスしても記法を実テキストとして挿入しない（[no-focus-expand.md](./no-focus-expand.md)）ため、シグネチャは本文テキストをそのまま比較すればよい（かつては挿入された記法文字の除外が必要で、カーソルを置いただけのブロックが「変更（青）」になる不具合を繰り返し生んでいた） |
| Undo / Redo | VS Code 標準のテキスト履歴 |
| フォーカス時記法表示（既定 off） | `preview.showFocusSyntax` を on にすると、カーソルがある行だけ行頭マーカー（`## ` `- ` `1. ` `> `）と行内記法（`**` `*` `` ` `` `~~` `[..](..)`）を薄字の widget で表示する。**既定は off** — フォーカスで見た目が変わらないことを既定の体験にするため（[no-focus-expand.md](./no-focus-expand.md) §2.3）。widget 表示なのでドキュメント本文は変わらない。<br>**マーカーの `<span>` は `contenteditable="false"`**（`createSyntaxMarkerElement`）。これが無いとエディタの `contenteditable=true` を継承して矢印キーのキャレットがマーカー文字の中に入り込む。<br>**マーカーをクリックするとカーソルがその位置へ移動する**（`data-pm-pos` 属性の位置へ `TextSelection` を dispatch）。<br>フェンスコードブロックの `` ``` `` は表示対象外（背景と等幅フォントで十分に区別できるため）。詳細: [code-fence-focus-markers.md](./code-fence-focus-markers.md) |
| フォーカスしても見た目が変わらない | カーソルをどのブロックへ移動しても、ドキュメント本文は 1 文字も変わらない。Markdown 記法は実テキストとして挿入されず、常に「記法は見えず、スタイルだけが付いた」表示になる。かつては Typora 風の**フォーカス展開**（`blockPrefixEditPlugin` / `inlineMarkEditPlugin` / `codeFenceEditPlugin` が`## ` `` ` `` `` ``` `` 等を実テキストとして挿入し、フォーカスが外れると削除する）を持っていたが、「フォーカスで本文が変わる」こと自体がカーソル飛び・差分ガター誤判定・行番号ズレ・保存内容の破損といった不具合の温床だったため 2026-07-26 に廃止した。記法を**外す**操作は行頭・マーク端の Backspace でリアルタイムに反映され、記法を**付ける**操作はショートカット・ツールバー・スラッシュメニュー・入力ルールで行う。詳細: [no-focus-expand.md](./no-focus-expand.md) |
...
Preview は記法を実テキストとして展開しないため、Cmd/Ctrl+← はブラウザ既定の行頭移動に委ねる（Preview の行頭＝ソースの本文先頭が一致するので、2 段階移動は不要になった）。
