...
| フォーカス時記法表示 | カーソルブロックで**行内**の Markdown 記法（`**` `*` `` ` `` `~~` `[..](..)`）を本文と同色の薄字で表示（`preview.showFocusSyntax`）。行頭マーカー（`## ` `- ` `- [ ] `）は表示しない（行頭 widget がカーソルの左移動を妨げ不自然になるため） |
...
| コードブロック言語選択 | コードブロック右上のドロップダウンで言語（`bash`/`js`/`python` 等）を選択・変更。`language` 属性に保存（` ```js ` 等） | 自動 |
