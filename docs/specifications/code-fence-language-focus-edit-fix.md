...
> **2026-07-09 追記**: `code-fence-real-text-edit-fix.md` により、フェンス自体
> （`` ```lang `` の部分も含む）が実テキストとして直接編集できるようになった。
> 本仕様のフロート `<input>` は変更・削除しておらず、そのまま並行して動作する
> （どちらで編集しても `language` 属性を更新する。同時編集時は最後の変更が勝つ、
> 単純な後勝ち。詳細は `code-fence-real-text-edit-fix.md` §2「言語入力欄との関係」）。
