...
| フォーカス時記法表示       | カーソルがあるブロックで見出し・強調・リンク等の Markdown 記法を表示           |
| 見出し Backspace 降格 | 見出し先頭で Backspace → `#テキスト` の通常行へ（`#` は続けて削除可能）    |
| スラッシュメニュー        | `/` で Raw と同様のコマンド一覧（h1–h6, table, code, quote 等） |
...
7. **共同編集プレビュー** — 同期基盤完成後のライブ更新表示（未実装）
