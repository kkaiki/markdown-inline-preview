 ...
 * テーブルデータを解析
 */
function parseTableRows(document, startLine, endLine) {
    const rows = [];
    let maxCols = 0;
    for (let i = startLine; i <= endLine; i++) {
        ...
        if (!cells || cells.length === 0)
            continue;
        rows.push({ line: i, cells, isSep: (0, patterns_1.isSeparatorRow)(cells) });
 ...
 * テーブル行をフォーマット
 */
function formatTableRow(row, colWidths, colHasFullWidth, maxCols) {
    let out = '|';
    if (row.isSep) {
        ...
        for (let c = 0; c < maxCols; c++) {
            ...
            const padded = (0, width_1.padCell)(cell, colWidths[c], colHasFullWidth[c]);
            out += padded + '|';
        }
    }
    return out;
 ...
 * 列幅を計算
 */
function calculateColumnWidths(rows, maxCols) {
    ...
    for (const r of rows) {
        ...
        for (let c = 0; c < maxCols; c++) {
            ...
            const w = Math.max(5, (0, width_1.getDisplayWidthWithHeuristics)(cell) + 2);
            if (w > colWidths[c])
                colWidths[c] = w;
            if (!colHasFullWidth[c] && cell) {
                const hasFullWidth = [...cell].some(ch => {
                    const cp = ch.codePointAt(0);
                    return cp !== undefined && (0, width_1.isFullWidthCodePoint)(cp);
                });
                if (hasFullWidth) {
