...
- Fix: Placing the cursor in a line containing inline markup — `` `code` ``, `**bold**`, `[text](url)` — made the Git-diff gutter mark the block as "modified" (blue bar) even though nothing had been edited; inside a table cell the bar spanned the whole table, since diff granularity is the top-level node. Same class as the `blockPrefixEditPlugin` fix below, but for `inlineMarkEditPlugin`, which inserts the marker characters as real text on focus. `blockSignatures()` now takes a *list* of expanded ranges and excludes both the block prefix (`## `, `- `, `> `) and every inline marker range (`getExpandedInlineMarkRanges()`). `expandBlock()` also commits its expansion state *before* dispatching, because otherwise the diff plugin's `decorations()` runs during that transaction with an empty range list and the bar stays until the next state change. Marker ranges are recomputed against the live document with the same "is the marker still intact" checks collapse uses, so partially deleting a marker mid-edit can't shift the excluded range onto real text. The same false positive existed for `codeFenceEditPlugin`, which inserts the ```` ```lang ```` / ```` ``` ```` fence lines as real text when a code block gets focus; its ranges (`getExpandedCodeFenceRanges()`, validated with the same `parseCodeFenceRealText` check collapse uses) are now excluded too — those three plugins are the complete set that inserts real Markdown text on focus. Guarded by `test/webview/focus-expand/previewDiffInlineMarkExpand.integration.test.ts` (9 tests) and, on the real bundle with real clicks, `test/browser/external-sync/diffGutterFocusExpand.test.ts` (7 tests — table cell, paragraph, heading whose prefix and inline code expand at once, and the inverse check that typing still marks the block).
- Change: Preview now materializes every blank line in the source, one visible empty block per blank line (N blank lines → N empty paragraphs), so Preview rows and Raw lines stay 1:1 and the line-number gutter never skips a number. Earlier the same day this was changed to N-1 ("a single blank line is just the paragraph separator, so it needs no node"), but Preview paragraphs are styled `margin: 0`, which means an unmaterialized blank line has nowhere to appear: opening a file whose heading was followed by three blank lines showed only two of them and numbered the gutter `1, 3, 4, 5`. Reverted `blankLineRemarkPlugin`, the `collapseBlankLineChains` round-trip, the gutter's blank-line interpolation, and the hardbreak→block split used by Enter-then-Markdown-conversion (which again leaves an empty placeholder paragraph, so the view right after converting matches the view after reopening the saved file). The guiding rule, in the user's words: a preview that doesn't omit anything and isn't over-styled. See `docs/specifications/blank-line-preservation.md` §10; guarded by `test/browser/rendering/blankLineDisplay.test.ts`.
- Add: `markdownInline.preview.controlDefaultEditor` (default **on**) — keeps VS Code's default editor for `*.md` / `*.markdown` in sync with the current mode by writing `workbench.editorAssociations`. Until now, `customEditors[0].priority: "default"` meant Raw-mode users always got the Preview custom editor created first and then bounced back to a text editor (`bounceToRawEditor`), which is what produced the flicker and the moment where two editors existed for one URI; it also left the resolution ambiguous whenever another installed extension claimed `*.md` as its own default editor (e.g. `cweijan.vscode-office`'s `cweijan.markdownViewer`), since `priority` is only a declaration and cannot break a tie. A user-level editor association outranks any extension's `priority`, so switching to Raw now makes VS Code resolve the plain text editor directly — the Preview editor is never constructed — and switching back to Preview restores the association. Unrelated associations (`*.pdf`, …) are preserved, and turning the setting off removes only the entries this extension wrote. `bounceToRawEditor` stays as the fallback for opt-out, `untitled:` documents, and the brief window before a settings write lands. See `docs/specifications/default-editor-association-sync.md`.
- Fix: Opening a Markdown file while Raw mode was the remembered/default mode could fail outright with `Unable to open 'x.md' / OverlayWebview has been disposed`, leaving a broken custom-editor tab that never became a text editor. `bounceToRawEditor` called `webviewPanel.dispose()` from inside `resolveCustomTextEditor`; VS Code treats a panel disposed mid-resolution as a failed open. It now lets the resolution finish (`setTimeout(0)`), replaces the tab via `vscode.openWith(uri, 'default')`, and leaves disposal to VS Code — cleaning up a leftover Preview tab only when the replacement didn't happen in place (untitled documents). Guarded by `test/extension/preview/tabs-editors.test.ts` 17.2.
...
- Test: All test titles across every tier are now auto-collected into `docs/testing/preview-test-catalog.md` (a living use-case spec; regenerate with `npm run docs:test-catalog`), and `docs/testing/spec-test-coverage.md` maps each spec document to the tests that guard it (with known gaps listed). The extension-host runner also accepts `MOCHA_GREP` to run a subset of the real-VS-Code tests.
